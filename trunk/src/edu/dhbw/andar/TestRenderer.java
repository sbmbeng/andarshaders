...
class TestRenderer implements Renderer, PreviewFrameSink {
    ...
    public void onDrawFrame(GL10 glUnused) {
        ...
        Log.i("POOPOOP", "Drawing Frame!, FrameEnqueued = " + frameEnqueued);
        
        if (this.frameEnqueued) {
        	Log.i("POOPOOP", "Frame has been enqueued");
			frameLock.lock();
			if(!isTextureInitialized) {
				initializeTexture(glUnused);
			} else {
				...
				Log.i("POOPOOP", "FrameWidth: " + previewFrameWidth + " FrameHeight: " + previewFrameHeight + " frameData: " + frameData.toString());
				checkGlError("glTexSubImage2D");
			}
			frameLock.unlock();
	        Log.i("POOPOOP", "Setting frameEnqueued to false");
			this.frameEnqueued = false;
		}
        
        /*
        try{
        	InputStream is = mContext.getAssets().open("shaders/robot.png");
        	Bitmap bitmap;
            bitmap = BitmapFactory.decodeStream(is);
            GLUtils.texImage2D(GLES20.GL_TEXTURE_2D, 0, bitmap, 0);
            bitmap.recycle();
        } catch(IOException e) {
        	Log.i( TAG, "Could not load the texture!" );
        }
        */
	...
	protected static FloatBuffer makeFloatBuffer(float[] arr) {
		Log.i("AndARRenderer", "Making a float buffer");
	...
	private void initializeTexture (GL10 gl) {
		Log.i("TestRenderer", "Initializing Texture");
		byte[] frame;
		switch(mode) {
		default:
			mode = GLES20.GL_RGB;
