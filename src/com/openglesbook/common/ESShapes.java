...
public class ESShapes {
	...
	public int genSphere(int numSlices, float radius) {
		...
		mVertexData = new float[ numVertices * 3 ];
		int idx = 0;
		...
		for (i = 0; i < numParallels + 1; i++) {
			for (j = 0; j < numSlices + 1; j++) {
				...
				mVertexData[idx++] = (float) (radius * Math.sin(angleStep * (float) i) * Math.sin(angleStep * (float) j));

				mVertices.put(vertex + 1,
						(float) (radius * Math.cos(angleStep * (float) i)));
				mVertexData[idx++] = (float) (radius * Math.cos(angleStep * (float) i));
				mVertices
						.put(vertex + 2,
								(float) (radius
										* Math.sin(angleStep * (float) i) * Math
										.cos(angleStep * (float) j)));
				mVertexData[idx++] = (float) (radius * Math.sin(angleStep * (float) i) * Math.cos(angleStep * (float) j));
	...
	public int genCube(float scale) {
		...
		for (i = 0; i < numVertices; i++) {
			...
			mVertexData[idx++] = mVertices.get(i) * scale;
		}
